...
### ⚠️ 중요: 수동 버전 동기화 원칙
이 프로젝트는 Expo Bare Workflow를 사용하므로, 버전 업데이트 시 `app.json`만 수정해서는 안 되며 **아래의 5가지 파일(Android/iOS/JS)의 버전을 반드시 일치**시켜야 합니다.
...
- `defaultConfig.versionCode`: `app.json`의 `versionCode`와 일치해야 함 (예: 34)
- `defaultConfig.versionName`: `app.json`의 `version`과 일치해야 함 (예: "1.3.3")

#### 3. 안드로이드 리소스 설정: `android/app/src/main/res/values/strings.xml`
- `<string name="expo_runtime_version">`: `app.json`의 `runtimeVersion`과 **반드시** 일치해야 함. (예: "35") 
- *주의: 이것이 다르면 네이티브 앱이 현재 EAS Update 파티션을 찾지 못해 OTA 업데이트가 실패합니다.*

#### 4. iOS 빌드 설정: `ios/CSHCHATAPP.xcodeproj/project.pbxproj`
- `MARKETING_VERSION`: `app.json`의 `version`과 일치해야 함 (예: "1.3.4")
- `CURRENT_PROJECT_VERSION`: `app.json`의 `ios.buildNumber`와 일치해야 함 (예: "10")
- `PRODUCT_BUNDLE_IDENTIFIER`: `app.json`의 `bundleIdentifier`와 일치해야 함 (예: "com.cshchatapp")
