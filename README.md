...
- Firebase 기반의 React Native 1:1/그룹 채팅 앱으로, 개인의 혹은 소규모 모임(단체)의 커뮤니케이션 도구로 개발되었습니다.
- 기본적인 개인 간 채팅 기능을 중심으로 설계되었으며, 실제 서비스에서 발생할 수 있는 네트워크가 불안정한 환경에서도 UX를 최적화하기 위해 SQLite, React Query 기반의 데이터 캐싱 및 최적화 구조를 적용했습니다.
...
│  │  ├─ assets             # 이미지/폰트 등 리소스
│  │  ├─ constants          # 색상/환경값/키 상수 등
│  │  ├─ firebase           # Firestore/FCM 래퍼, 공통 쿼리/구독 유틸
│  │  ├─ hooks              # feature에 종속되지 않는 공용 훅
│  │  ├─ sqlite             # 로컬 DB 스키마/쿼리/캐시 레이어
│  │  ├─ types              # 전역 타입(공통 DTO, 유틸 타입)
│  │  ├─ ui                 # 디자인 시스템/공용 UI 컴포넌트(입력폼, 모달, 그에 관련된 훅 등)
│  │  └─ utils              # 공용 유틸(포맷터, 로거, 에러 처리 등)
│  └─ store                 # 전역 상태(로그인 유저, 앱 설정 등, React Query가 있어서 최소한만 사용)
├─ settings.gradle
└─ App.tsx                 # App.tsx / index.js 등 프로젝트 엔트리(현재 구성에 맞게 표기)
